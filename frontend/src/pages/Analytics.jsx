...
import {
  TrendingUp, Users, FileText, Receipt, CheckCircle2,
  Clock, AlertCircle, Percent, Target, PartyPopper, ArrowRightLeft
} from "lucide-react";
...
export default function Analytics() {
  ...
  const data = useMemo(() => {
    ...
    const leadConversion = contacts.length ? Math.round((won.length / contacts.length) * 100) : 0;
    const wonRevenue = won.reduce((s, l) => s + (parseFloat(l.estimatedValue || l.estimated_value) || 0), 0);

    const bySource = LEAD_SOURCES.map(s => ({
      ...s,
      count: contacts.filter(c => c.leadSource === s.id || c.lead_source === s.id).length,
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const total = paid
        .filter(p => {
          const pd = new Date(p.paid_at || p.paidAt || p.created_at);
    ...
    return {
      ...
      allLeads: contacts, won, lost, open, leadConversion, wonRevenue,
      bySource, srcMax,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <Card className="p-6">
          ...
          <div className="space-y-4">
            {[
              { label: "Paid",    val: data.paid.length,    color: "bg-emerald-500" },
              { label: "Pending", val: data.pending.length, color: "bg-amber-500" },
              { label: "Overdue", val: data.overdue.length, color: "bg-rose-500" },
              { label: "Draft",   val: data.draft.length,   color: "bg-muted-foreground/40" },
            ].map(row => (
              <div key={row.label}>
                ...
                <Bar value={row.val} max={Math.max(1, invoices.length)} color={row.color} />
              </div>
            ))}
          </div>
        </Card>
